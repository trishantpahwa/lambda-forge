...
const ROLE_NAME      = 'lambda-forge-execution-role';
...
module.exports = async function deploy(args) {
    ...
    if (!fs.existsSync(appsDir)) {
        console.error('  Error: No "apps" directory found. Run from a lambda-forge project.\n');
        process.exit(1);
    }
...
async function ensureRole(iam) {
    ...
    const { Role } = await iam.send(new CreateRoleCommand({
        RoleName: ROLE_NAME,
        Description: 'Lambda execution role managed by lambda-forge',
...
function buildZip(projectDir, appName) {
    return new Promise((resolve, reject) => {
        ...
        // Always bundle the running lambda-forge source directly so the Lambda
        // runtime uses the exact same version as the CLI, not whatever npm installed.
        const forgeRoot = path.join(__dirname, '..');
        archive.file(path.join(forgeRoot, 'package.json'), { name: 'node_modules/lambda-forge/package.json' });
        archive.directory(path.join(forgeRoot, 'lib'), 'node_modules/lambda-forge/lib');

        // Add every node_modules entry from the project except lambda-forge,
        // which is already bundled from source above.
        const nodeModulesDir = path.join(projectDir, 'node_modules');
        if (fs.existsSync(nodeModulesDir)) {
            for (const entry of fs.readdirSync(nodeModulesDir)) {
                if (entry === 'lambda-forge') continue;
