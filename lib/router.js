class Router {
    ...
    handle(req, res, basePath = '') {
        ...
        // Normalize trailing slash so /foo/ and /foo match the same route
        const normalized = pathname.length > 1 && pathname.endsWith('/')
            ? pathname.slice(0, -1)
            : pathname;

        const localPath = normalized.startsWith(basePath)
        ...
        if (!route) {
            res.statusCode = 404;
            res.end(`Cannot ${req.method} ${normalized}`);
            return;
        }
    ...
    _matchPath(routePath, actualPath) {
        ...
        return rParts.every((part, i) => {
            if (part.startsWith(':')) return aParts[i].length > 0;
            return part === aParts[i];
        });
    }
    ...
    _sendError(res, err) {
        console.error('lambda-press error:', err);
        try {
            res.statusCode = 500;
