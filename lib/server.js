...
// Redirect require('lambda-forge') in loaded app files to this running package,
// so projects work without needing lambda-forge published or linked in node_modules.
const _resolveFilename = Module._resolveFilename.bind(Module);
Module._resolveFilename = (request, parent, isMain, options) => {
    if (request === 'lambda-forge') return require.resolve('./index');
...
module.exports = function serve(args = []) {
    ...
    if (!fs.existsSync(appsDir)) {
        ...
        console.error('  Make sure you are in a lambda-forge project directory.\n');
        process.exit(1);
    }
